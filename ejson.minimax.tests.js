...
Tinytest.add('Minify Maxify - test', function(test) {
  ...
  var minResult = EJSON.minify(myObject);

  var maxResult = EJSON.maxify(minResult)

  var savedBytes = myJSON.length - minResult.length;

  console.log(myJSON.length + ' > ' + minResult.length + ' '+ Math.round(savedBytes/myJSON.length*100) + "%");


  test.isTrue(EJSON.equals(maxResult, myObject));
...
console.log(test2);
  ...
  minResult = EJSON.minify(test2);

  maxResult = EJSON.maxify(minResult);

  ejsonResult = EJSON.stringify(test2);
